...
- **보기**: 각 선택지. 내용을 요약·생략하지 말고 **원문 그대로 보존하되 한국어로 번역**한다 (서비스명·포트·속성명은 원문 유지)
...
1. **문제와 보기를 원본 그대로(빠짐없이) 보존하되, 한국어로 번역할 것.** 내용을 요약·생략·재구성하지 말고 모든 문장을 그대로 옮기되, 한국어로 자연스럽게 번역한다 (서비스명·포트·속성명은 원문 유지).
...
- 영어 원문은 한국어로 번역하되 **서비스명(SQS, Kinesis…), 포트 번호, 속성명(Group ID…)은 원문 유지**. 문제·보기는 내용을 빠짐없이 보존한 채 번역한다.
